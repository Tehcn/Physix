...
class RLGymExampleBot(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        if self.update_action:
            ...
            opponents = [p for p in self.game_state.players if p.team_num != self.team and p.car_id != self.index]

            if len(opponents) != self.expected_opponents or len(teammates) != self.expected_teamates:
                self.reshape(self.game_state)
