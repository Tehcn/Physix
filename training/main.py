...
if __name__ == '__main__':  # Required for multiprocessing
    ...
    callback = CheckpointCallback(round(mmr_save_frequency / env.num_envs), save_path="models", name_prefix="rl_model")
    ...
    try:
        ...
        while True:    
            ...
            if model.num_timesteps >= mmr_model_target_count:
                print(f"[{time.time()}] Saving model to models/{model.num_timesteps}.zip")
                model.save(f"mmr_models/{model.num_timesteps}")
                mmr_model_target_count += mmr_save_frequency
            if model.num_timesteps % 250_000 >= save_frequency:
                import time
                print(f"[{time.time()}] Saving model to models/exit_save.zip")
                exit_save()
